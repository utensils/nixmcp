#!/usr/bin/env python
"""
NixMCP Server - An enhanced MCP server for NixOS resources.

This implements a comprehensive FastMCP server that provides resources and tools
...
class ElasticsearchClient:
    ...
    def __init__(self):
        ...
        self.es_packages_url = "https://search.nixos.org/backend/latest-42-nixos-unstable/_search"
        self.es_options_url = "https://search.nixos.org/backend/latest-42-nixos-unstable-options/_search"
        ...
        self.es_aws_endpoint = "https://nixos-search-5886075189.us-east-1.bonsaisearch.net:443"
                    ...
                    "query": {
                        "bool": {
                            "should": [
                                ...
                                {"wildcard": {"package_pname": {"value": f"*{term}*", "boost": 7}}},
                                {"match": {"package_description": {"query": term, "boost": 3}}},
                ...
                "query": {
                    "bool": {
                        "should": [
                            ...
                            # Contains match
                            {"wildcard": {"package_attr_name": {"value": f"*{query}*", "boost": 5}}},
                            ...
                            {"match": {"package_longDescription": {"query": query, "boost": 1}}},
                ...
                "query": {
                    "bool": {
                        "should": [
                            ...
                            {"wildcard": {"option_name": {"value": f"*{query}*", "boost": 4}}},
                            # Full-text search in description
                            {"match": {"option_description": {"query": query, "boost": 2}}}
                        ],
                        "minimum_should_match": 1
                    }
                },
                ...
                "query": {
                    "bool": {
                        "should": [
                            ...
                            {"wildcard": {"package_programs": {"value": f"*{program}*", "boost": 3}}}
                        ],
                        "minimum_should_match": 1
                    }
                }
    ...
    def search_packages_with_version(self, query: str, version_pattern: str, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        """
    ...
    def get_package_stats(self, query: str = "*") -> Dict[str, Any]:
        ...
        request_data = {
            ...
            "aggs": {
                "channels": {
                    "terms": {"field": "package_channel", "size": 10}
                },
                "licenses": {
                    "terms": {"field": "package_license", "size": 10}
                },
                "platforms": {
                    "terms": {"field": "package_platforms", "size": 10}
                }
            }
        }
...
@mcp.tool()
def advanced_search(query_string: str, index_type: str = "packages", limit: int = 20) -> str:
    """
...
def package_statistics(query: str = "*") -> str:
    ...
    try:
        ...
        output += f"- Cache size: {cache_stats['size']}/{cache_stats['max_size']} entries\n"
...
def version_search(package_query: str, version_pattern: str, limit: int = 10) -> str:
    ...
    Returns:
        Search results formatted as text
    """
    logger.info(f"Searching for packages matching '{package_query}' with version '{version_pattern}'")
    
    try:
        results = model_context.search_packages_with_version(package_query, version_pattern, limit)

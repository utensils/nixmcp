...
from server import ElasticsearchClient, NixOSContext, create_wildcard_query
...
class TestHierarchicalPathQueries(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_es_query.return_value = {
            "hits": {
                "total": {"value": 0},
                "hits": []
            }
    ...
    def test_service_path_query_structure(self):
        ...
        if hasattr(bool_query["must"][0], "get") and bool_query["must"][0].get("bool"):
            ...
            self.assertTrue(has_prefix or has_wildcard, 
...
class TestQueryBoostingValues(unittest.TestCase):
    ...
    def test_service_path_boost_values(self):
        ...
        self.assertGreaterEqual(len(boost_values), 2, 
                               "Query should contain at least 2 boost values")
        
        # Check that we have a range of boost values (some should be higher than others)
        self.assertGreater(max(boost_values), min(boost_values),
